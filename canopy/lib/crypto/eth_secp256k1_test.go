...
func TestEthSECP256K1Bytes(t *testing.T) {
	// private key testing
	...
	fmt.Println(len(pubKeyBz), hex.EncodeToString(pubKeyBz))
	pubKey2, err := BytesToEthSECP256K1Public(pubKeyBz)
	require.NoError(t, err)
...
func TestEthSECP256K1SignAndVerify(t *testing.T) {
	// create the private key
	pk, err := NewETHSECP256K1PrivateKey()
	if err != nil {
		t.Fatal(err)
	}
