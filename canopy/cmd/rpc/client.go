...
// TransactionByHashRaw returns the raw JSON response for a transaction query,
// avoiding typed deserialization issues with hex/base64 encoding of byte fields
func (c *Client) TransactionByHashRaw(hash string) (result map[string]any, err lib.ErrorI) {
	result = make(map[string]any)
	err = c.hashRequest(TxByHashRouteName, hash, &result)
	return
}
