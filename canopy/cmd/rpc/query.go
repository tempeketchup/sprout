...
// PluginPrefix iterates over a prefix and returns the raw bytes
func (s *Server) PluginPrefix(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	if err := r.ParseForm(); err != nil {
		write(w, err, http.StatusBadRequest)
		return
	}
	prefixHex := r.Form.Get("prefix")
	...
	s.readOnlyState(0, func(state *fsm.StateMachine) lib.ErrorI {
		res, err := state.IterateAndAppend(prefix)
		if err != nil {
			return err
		}
		var hexRes []string
		for _, b := range res {
			hexRes = append(hexRes, hex.EncodeToString(b))
		}
		write(w, hexRes, http.StatusOK)
		return nil
	})
}
