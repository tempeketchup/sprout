...
var (
	ksCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			ks, err := client.Keystore()
			if err != nil {
				l.Fatal(err.Error())
			}
			keys := make([]map[string]any, 0)
			for _, key := range ks.AddressMap {
				isValidator := false
				_, vErr := client.Validator(0, key.KeyAddress)
				if vErr == nil {
					isValidator = true
				}
				keys = append(keys, map[string]any{
					"address":     key.KeyAddress,
					"nickname":    key.KeyNickname,
					"isValidator": isValidator,
				})
			}
			writeToConsole(map[string]any{"keys": keys}, nil)
		},
	}
	...
	ksNewKeyCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			address, err := client.KeystoreNewKey(getPassword(), getNickname())
			if err != nil {
				l.Fatal(err.Error())
			}
			writeToConsole(map[string]any{
	...
	txSendCmd = &cobra.Command{
		Use:     "tx-send <address or nickname> <to-address or nickname> <amount> --fee=10000 --simulate=true",
		...
		Run: func(cmd *cobra.Command, args []string) {
			toAddr := resolveAddrOrNickname(args[1])
			writeTxResultToConsole(client.TxSend(argGetAddrOrNickname(args[0]), toAddr, uint64(argToInt(args[2])), getPassword(), !sim, fee))
		},
	}
...
// resolveAddrOrNickname tries to parse arg as a hex address first; if that fails,
// it queries the keystore to resolve the nickname to an address.
func resolveAddrOrNickname(arg string) string {
	bz, err := lib.StringToBytes(arg)
	if err == nil && len(bz) == crypto.AddressSize {
		return arg
	}
	// Not a valid hex address — treat as nickname and resolve via keystore
	ks, ksErr := client.Keystore()
	if ksErr != nil {
		l.Fatalf("failed to query keystore to resolve nickname %q: %s", arg, ksErr.Error())
	}
	addr, ok := ks.NicknameMap[arg]
	if !ok {
		l.Fatalf("nickname %q not found in keystore", arg)
	}
	return addr
}
