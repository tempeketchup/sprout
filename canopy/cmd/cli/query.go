...
var (
	...
	accountCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			account, err := client.Account(height, args[0])
			if err != nil {
				l.Fatal(err.Error())
			}
			var stakedAmount uint64
			val, vErr := client.Validator(height, args[0])
			if vErr == nil && val != nil {
				stakedAmount = val.GetStakedAmount()
			}
	...
	txByHashCmd = &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			raw, err := client.TransactionByHashRaw(args[0])
			if err != nil {
				l.Fatal(err.Error())
			}
			...
			if tx, ok := raw["transaction"].(map[string]any); ok {
				result["fee"] = tx["fee"]
				if msg, ok := tx["msg"].(map[string]any); ok {
					result["amount"] = msg["amount"]
				}
			}
			writeToConsole(result, nil)
		},
	}
